...
def _load_moondream():
    ...
    if _moondream_model is None:
        import sys
        print("[eyes] Loading Moondream2 locally via transformers...", file=sys.stderr)
        from transformers import AutoModelForCausalLM, AutoTokenizer
        _moondream_model = AutoModelForCausalLM.from_pretrained(
            _MOONDREAM_MODEL_ID, trust_remote_code=True, revision=_MOONDREAM_REVISION
        ).to("cuda") # Keeps it entirely on VRAM since it's only ~2GB and fast
        _moondream_tokenizer = AutoTokenizer.from_pretrained(
            _MOONDREAM_MODEL_ID, revision=_MOONDREAM_REVISION
        )
