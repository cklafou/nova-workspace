# Last updated: 2026-08-04 22:51:21
"""
...
async def stream_response(
    ...
    try:
        ...
        _escalated_this_turn = False  # binding cloud arbitration fired this turn?
                                      # one-shot; after it, her next answer is final.
        ...
        # ── HOLD-BACK STREAMING (2026-08-03, Cole's turnabout redesign) ──
        ...
        while loop_counter < max_loops:
            ...
            async def token_handler(token: str):
                ...
                # HELD: draft buffered in full_response/_streamed and NOT shown; only the
                # witness-cleared final answer ships, via on_done → message_end below.
            ...
            if _gate_why:
                ...
                if _concern and _witness_rounds < _WITNESS_MAX_ROUNDS and not _deadlocked:
                    ...
                    try:
                        ...
                        # ── BINDING ARBITRATION + CONSENSUS TURN (Cole, 2026-08-03 turnabout) ──
                        # A checkable-fact dispute the local witness could not settle: ask the
                        # cloud arbiter WHO IS RIGHT and ACT on it (not logs-only). Ping-verify +
                        # ~8s live in cloud_call. Reachable + rules in time -> BINDING. Any skip ->
                        # Nova gets ONE consensus turn as the FINAL AUTHORITY (keep or revise, her
                        # call) — the anti-gaslighting backstop: the witness never forces a
                        # concession she does not accept. One-shot (_escalated_this_turn); fail-safe:
                        # any trouble degrades to a skip -> her consensus turn. Knob
                        # binding_cloud_escalation=false reverts to the proven background lane.
                        async def _cloud_verdict(_cv_draft=chat_text, _cv_concern=_concern,
                                                 _cv_tools=list(_turn_tools),
                            ...
                            try:
                                ...
                                from pathlib import Path as _P
                                _ccp = (_P(__file__).resolve().parents[2]
                                        / "general_tools" / "cloud_call.py")
                                _spec = _ilu.spec_from_file_location("nova_cloud_call", _ccp)
                                _cc = _ilu.module_from_spec(_spec)
                                _spec.loader.exec_module(_cc)
                            except Exception as _e:
                                print(f"[nova] cloud arbiter unavailable (fail-open): {_e}")
                                return "skip"
                            _loop = asyncio.get_running_loop()
                            try:
                                _v = await _loop.run_in_executor(
                                    None, lambda: _ctx.run(lambda: _cc.heavy_witness(
                            ...
                            _side = {"her": "sides with HER — her answer stands",
                                     ...
                                     "skip": "no ruling in time — Nova decides"}[_outcome]
                            try:
                                _witness.pipeline_event(
                                    "witness_heavy",
                                    f"BINDING cloud arbitration "
                                    f"({_cv_stage.replace('witness_', '')} after "
                                    f"{_cv_rounds} round(s)): {_side} [{_dt:.0f}s]",
                                    draft=_cv_draft, inline_concern=_cv_concern, verdict=_v,
                                    outcome=_outcome, binding=True, latency_s=round(_dt, 1),
                                    history_turns=len(_cv_history), tools_seen=len(_cv_tools),
                                    heavy_log=("logs/heavy_witness/" + _t_cv.strftime(
                                        "%Y-%m-%d", _t_cv.localtime()) + ".jsonl"))
                            except Exception as _ee:
                                print(f"[nova] binding arbiter event failed: {_ee}")
                            return _outcome

                        _binding = (_tune("binding_cloud_escalation", True)
                                    and not _escalated_this_turn
                                    and _tune("heavy_witness_enabled", True)
                        ...
                        if _binding:
                            ...
                            try:
                                ...
                                print(f"[nova] binding arbitration crashed (fail-open): {_abe}")
                                _arb = "skip"
                            if _arb == "concern":
                                try:
                                    _witness.pipeline_event(
                                        "cloud_binding",
                                        "the cloud arbiter read the whole conversation "
                                        "and sided with the WITNESS — she revises to "
                                        "address it", concern=_concern, draft=chat_text)
                                except Exception:
                                    pass
                                messages.append({"role": "assistant", "content": full_response})
                                messages.append({"role": "user", "content": (
                                    "[The cloud arbiter — it had the whole conversation in "
                                    "front of it — was asked to settle the dispute between "
                                    "your reply and the witness, and it sided with the witness. "
                            ...
                            if _arb == "skip":
                                try:
                                    _witness.pipeline_event(
                                        "consensus_turn",
                                        "cloud unreachable in time — Nova decides as "
                                        "the FINAL AUTHORITY on this concern (keep or "
                                        "revise, her call)", concern=_concern, draft=chat_text)
                                except Exception:
                                    pass
                                messages.append({"role": "assistant", "content": full_response})
                                messages.append({"role": "user", "content": (
                                    "[The witness raised a concern about your reply, and the "
                                    "cloud arbiter could not be reached in time to settle it "
                                    "— so this is YOUR call. You hold the most context here. "
                                    "The concern was:]" + chr(10) + chr(10) + _concern + chr(10) +
                                    chr(10) + "[Decide for yourself. If your reply is right as "
