"""
nova_cortex/agent_loop.py
===========================
The core inference loop: trigger → llama.cpp → tools → response.
...
What this does (plain English):
  ...
  conversation cycle:
    ...
    4. POST messages to llama.cpp's /v1/chat/completions endpoint
       (We talk to the standalone llama-server.exe so we don't load VRAM twice)
...
DEPENDENCY: llama-server.exe must be running at localhost:8080 for inference
to work. The gateway is a lightweight orchestration layer — it has no model
of its own.
"""
...
LLAMA_CPP_INFER_URL = "http://127.0.0.1:8080/v1/chat/completions"
...
async def run_agent(
    ...
    Args:
        ...
        on_token:             Optional streaming callback (str → None).
                              Called once with the full response text from nova_chat.
    ...
    for iteration in range(MAX_TOOL_ITERATIONS + 1):
        ...
        # Call llama.cpp inference endpoint natively
        response, usage, err = await _call_llamacpp(messages, stream_cb=on_token)
        if err:
            return AgentResult(
...
async def _call_llamacpp(
    ...
    """
    POST messages to standalone llama.cpp server.
    
    Returns: (response_dict, usage_dict, error_string_or_None)
    """
    payload = {
        "messages":   messages,
        "max_tokens": cfg.inference.get("max_tokens", 4096),
        "temperature": 0.7,
        "top_p": 0.9,
    }
    timeout = cfg.inference.get("timeout_s", 300)

    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.post(LLAMA_CPP_INFER_URL, json=payload)
            resp.raise_for_status()
            data  = resp.json()
            usage = data.get("usage", {})

            # Fire stream_cb once with full content if caller wants streaming
            if stream_cb:
        ...
        return {}, {}, f"llama.cpp inference timeout after {timeout}s"
    except httpx.ConnectError:
        return {}, {}, "Cannot reach llama-server connection. Is it running?"
    except httpx.HTTPStatusError as e:
        return {}, {}, f"llama.cpp infer HTTP {e.response.status_code}: {e.response.text[:200]}"
    except Exception as e:
        return {}, {}, f"llama.cpp infer error: {type(e).__name__}: {e}"
...
async def _compact_session(session: Session, system_prompt: str) -> Optional[str]:
    ...
    Falls back to a generic summary if nova_chat call fails.
    """
    ...
    response, _, err = await _call_llamacpp(compaction_messages)
    if err:
        log.warning("Compaction llama.cpp call failed: %s — using generic summary", err)
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        print("(Requires llama-server running at localhost:8080)")
