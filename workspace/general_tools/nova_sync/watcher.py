# Last updated: 2026-08-05 09:41:25
import re
import sys
import time
import subprocess
...
def _clear_stale_git_lock(watch_dir, max_age_s=30):
    ...
    try:
        _git = Path(watch_dir) / ".git"
        # 2026-08-03: sweep EVERY lock kind that can wedge a commit, not just index.lock. A
        # stale 0-byte HEAD.lock (left by a device-side git that could not unlink it on the
        # mount) froze every commit for ~8h; the old sweep only cleared index.lock so it never
        # self-healed. refs/**/*.lock covers a lock left mid ref-update.
        _locks = [_git / "index.lock", _git / "HEAD.lock", _git / "config.lock"]
        try:
            _locks += list((_git / "refs").rglob("*.lock"))
        except Exception:
            pass
        for lock in _locks:
            try:
                if lock.exists() and (time.time() - lock.stat().st_mtime) > max_age_s:
                    lock.unlink()
                    print(f"[{time.strftime('%H:%M:%S')}] Removed stale {lock.name} "
                          f"(it was blocking every commit — the working tree could not reach HEAD)")
            except Exception as _le:
                print(f"[watcher] could not clear {lock}: {_le}")
    except Exception as e:
